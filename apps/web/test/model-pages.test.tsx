...
describe("Model pages", () => {
  ...
  it("renders one full-width card per bound question", async () => {
    apiRequest.mockResolvedValue({
      ...
      source: {
        ...
        notion_root_page_url: "https://www.notion.so/root",
        notion_root_title: "模型",
        auto_sync_enabled: true,
        auto_sync_interval_seconds: 300,
        next_sync_at: "2026-08-09T00:05:00Z",
    ...
    expect(screen.getByRole("link", { name: /Q1/ })).toHaveAttribute(
      "href",
      `/projects/${projectId}/models/${questionId}`,
    );
  });
  ...
  it("defaults new question codes to the next Q number and allows a blank code", async () => {
    ...
    apiRequest.mockImplementation(
      async (path: string, options?: { body?: unknown; method?: string }) => {
        if (path.endsWith("/questions") && options?.method === "POST")
          return overview.questions[0];
        return overview;
      },
    );
    ...
    await waitFor(() =>
      expect(apiRequest).toHaveBeenCalledWith(
        `/projects/${projectId}/models/questions`,
        {
          body: {
            code: "Q2",
            title: "新模型",
  ...
  it("renders contiguous character additions and deletions without line numbers", async () => {
    apiRequest.mockImplementation(async (path: string) => {
      const artifactVersion = {
        ...
        size_bytes: 1024,
        status: "available",
        available_at: "2026-08-09T00:05:00Z",
        git_reference: null,
        created_by: "user",
      ...
      if (path.endsWith("/artifacts/artifact-1/versions/version-1/download"))
        return {
          ...
          transfer: {
            method: "GET",
            url: "/api/artifact-transfers/token.signature",
            headers: {},
            expires_at: "2026-08-09T00:10:00Z",
          },
        };
      if (path.endsWith("/artifacts/artifact-1/versions/version-1/previews"))
        return { items: [] };
      if (path.endsWith("/artifacts/artifact-1/versions"))
        return { items: [artifactVersion] };
      if (path.endsWith("/artifacts/artifact-1")) return artifactDetail;
      if (path.endsWith("/diff")) {
        return {
          question_id: questionId,
          from_snapshot_id: firstSnapshot,
          to_snapshot_id: secondSnapshot,
          ...
          blocks: [
            {
              ...
              operations: [{ kind: "unchanged", text: "模型假设" }],
            },
            {
              block_id: "p1",
              type: "paragraph",
              ...
              block: {
                ...
                type: "paragraph",
                text: "人口快速增长",
                rich_text: [{ text: "人口快速增长" }],
                children: [],
              },
              operations: [
                { kind: "unchanged", text: "人口" },
                { kind: "deleted", text: "稳定" },
                { kind: "added", text: "快速" },
                { kind: "unchanged", text: "增长" },
              ],
            },
          ],
        };
      ...
      return {
        question: {
          ...
          notion_page_url: "https://www.notion.so/page",
          position: 0,
          latest_snapshot_id: secondSnapshot,
          snapshot_count: 2,
          sync_status: "succeeded",
          created_at: "2026-08-09T00:00:00Z",
          updated_at: "2026-08-09T00:00:00Z",
        ...
        latest_snapshot: {
          ...
          question_id: questionId,
          previous_snapshot_id: firstSnapshot,
          title: "人口模型",
          content_hash: "a".repeat(64),
          summary: "",
          tags: [],
          captured_at: "2026-08-09T00:05:00Z",
          triggered_by: "user",
          created_at: "2026-08-09T00:05:00Z",
          metadata_updated_at: "2026-08-09T00:05:00Z",
          project_id: projectId,
          notion_page_id: "page",
          notion_page_url: "https://www.notion.so/page",
          outline: [
            { block_id: "heading-current", title: "状态定义", level: 2 },
            { block_id: "heading-ignored", title: "四级标题", level: 4 },
          ],
          blocks: [
            {
        ...
        snapshots: [
          ...
          },
          {
            ...
            triggered_by: "user",
            created_at: "2026-08-09T00:00:00Z",
            metadata_updated_at: "2026-08-09T00:00:00Z",
          },
        ],
      };
    });
    ...
    const { container } = render(
      <ModelQuestionPage questionId={questionId} />,
      { wrapper: Providers },
    );
    await waitFor(() =>
      expect(container.querySelector("td")?.textContent).toContain("参数"),
    );
    expect(
      container.querySelector('[data-model-equation="inline"] .katex'),
    ).toBeInTheDocument();
    expect(
      container.querySelector('[data-model-equation="block"] .katex-display'),
    ).toBeInTheDocument();
    ...
    expect(outline?.parentElement?.parentElement).toHaveClass(
      "max-h-[calc(100vh-2rem)]",
    );
    expect(
      container.querySelector('a[href="#model-heading-heading-current"]'),
    ).toHaveTextContent("状态定义");
    expect(
      container.querySelector("#model-heading-heading-current"),
    ).toHaveTextContent("状态定义");
    expect(screen.queryByText("四级标题")).not.toBeInTheDocument();
    expect(screen.getByText("mmdash fork").closest("a")).toHaveAttribute(
      "href",
      "https://github.com/imouup/mmdash-fork/tree/main",
    );
    expect(screen.getByText("同步区块内容")).toBeInTheDocument();
    await waitFor(() =>
      expect(container.querySelector('img[alt="结果图"]')).toHaveAttribute(
        "src",
        "/api/artifact-transfers/token.signature",
      ),
    );
    expect(
      screen.getByText("模型说明书").closest("button"),
    ).toBeInTheDocument();
    fireEvent.click(
      container.querySelector(
        'button[aria-label="查看图片 Artifact：结果图"]',
      )!,
    );
    await waitFor(() =>
      expect(
        container.querySelector('[role="dialog"][aria-label="文件详情"]'),
      ).toBeInTheDocument(),
    );
    expect(
      await screen.findByText("结果图.png", { selector: "h2" }),
    ).toBeInTheDocument();
    fireEvent.click(container.querySelector('button[aria-label="关闭"]')!);
    fireEvent.click((await screen.findByText("比较版本")).closest("button")!);

    expect(
      container.querySelector('select[aria-label="Diff 基准版本"]'),
    ).toHaveValue(firstSnapshot);
    expect(
      container.querySelector('select[aria-label="Diff 目标版本"]'),
    ).toHaveValue(secondSnapshot);
    await waitFor(() =>
      expect(apiRequest).toHaveBeenCalledWith(
        `/projects/${projectId}/models/questions/${questionId}/diff`,
        {
          query: {
            from_snapshot_id: firstSnapshot,
            to_snapshot_id: secondSnapshot,
    ...
    await waitFor(() =>
      expect(container.querySelector("[data-diff-change]")).toBeInTheDocument(),
    );
    expect(
      container.querySelector('h2 [data-diff-kind="unchanged"]')?.textContent,
    ).toBe("模型假设");
  ...
  it("shows the Artifact display name instead of the snapshot filename after a rename", async () => {
    ...
    await waitFor(() =>
      expect(
        container.querySelector('img[alt="重命名后的结果图"]'),
      ).toHaveAttribute("src", "/api/artifact-transfers/token.signature"),
    );
    expect(
      screen.getByText("重命名后的结果图").closest("figcaption"),
    ).toBeInTheDocument();
  ...
  it("shows the authorized Notion workspace and saves only public synchronization settings", async () => {
    ...
    apiRequest.mockImplementation(
      async (path: string, options?: { body?: unknown; method?: string }) => {
        if (path === settingPath && options?.method === "PATCH") {
          return {
            values: (options.body as { values: Record<string, unknown> })
              .values,
            version: 3,
            updated_at: "2026-08-09T00:00:00Z",
          };
        ...
        if (path === settingPath) {
          return {
            values: {
              ...
              refresh_token: "********",
              root_page_url:
                "https://www.notion.so/Test-00000000000040008000000000000001",
              auto_sync_enabled: true,
              auto_sync_interval_seconds: 300,
            },
            version: 2,
        ...
        if (path.endsWith("/models/notion/oauth"))
          return {
            available: true,
            connected: true,
        ...
        if (path.endsWith("/models")) {
          return {
            ...
            source: {
              ...
              notion_root_page_id: "root",
              notion_root_page_url: "https://www.notion.so/root",
              notion_root_title: "Test",
              auto_sync_enabled: true,
              auto_sync_interval_seconds: 300,
              next_sync_at: new Date(Date.now() + 300_000).toISOString(),
              sync_status: "succeeded",
    ...
    expect(
      await screen.findByText(/已授权 Workspace：研究组/),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "保存同步设置" }));

    await waitFor(() =>
      expect(apiRequest).toHaveBeenCalledWith(settingPath, {
        body: {
          values: {
            root_page_url:
              "https://www.notion.so/Test-00000000000040008000000000000001",
