...
export function BoxManagement() {
  ...
  const revoke = useMutation({
    mutationFn: ({ boxId, mode }: { boxId: string; mode: "drain" | "force" }) => experimentApi.revokeBox(boxId, mode),
    onError: (error) => toast.error(error.message),
    onSuccess: async (result) => {
      toast.success(result.mode === "drain" ? "Box 已进入等待撤销" : "Box 已强制撤销");
      await client.invalidateQueries({ queryKey: ["personal-boxes"] });
    },
  });
  ...
  return (
    <section aria-labelledby="box-management-title" className="space-y-5">
      <header>
        <h1 className="text-2xl font-semibold" id="box-management-title">Box 管理</h1>
      ...
      {!boxes.isLoading && !boxes.data?.items.length ? (
        <Card><CardContent className="py-10 text-center"><BoxIcon className="mx-auto mb-3 size-8 text-muted-foreground" /><p className="font-medium">尚未绑定 Box</p><p className="mt-1 text-sm text-muted-foreground">在要作为 Box 的机器上启动 mmdash-box，并按终端提示完成浏览器授权。</p></CardContent></Card>
      ) : null}
    </section>
  );
}
...
function BoxCard({ box, onRename, onRevoke, pending }: Readonly<{ box: Box; onRename: (name: string) => void; onRevoke: (mode: "drain" | "force") => void; pending: boolean }>) {
  ...
  const forceRevoke = () => {
    if (window.confirm("强制撤销会立即使 Box Token 失效，并将相关运行中实验标记为失败。确定继续吗？")) onRevoke("force");
  };
  return (
    <Card>
      <CardHeader>
        <div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><BoxIcon className="size-4" />{box.name}</CardTitle><CardDescription className="mt-1 font-mono">{box.box_id}</CardDescription></div><Badge>{box.status}</Badge></div>
      </CardHeader>
      <CardContent className="space-y-4">
        {box.legacy_reauthorization_required ? <p className="rounded-md bg-amber-500/10 p-3 text-xs text-amber-700">旧版 Project-scoped Box 必须重新完成账号级设备授权。</p> : null}
        <div className="grid grid-cols-2 gap-3 text-xs">
          <Metric label="版本" value={box.version} />
          <Metric label="任务负载" value={`${box.load.running_tasks}/${box.load.capacity}`} />
          <Metric label="Runtime" value={box.runtimes.map((runtime) => runtime.name).join(", ") || "未探测到"} />
          <Metric label="Project 分配" value={String(box.project_assignments.length)} />
          <Metric label="最近心跳" value={box.last_heartbeat_at ? new Date(box.last_heartbeat_at).toLocaleString() : "尚无"} />
          <Metric label="离线时间" value={box.offline_since ? new Date(box.offline_since).toLocaleString() : "—"} />
        </div>
        <div className="flex gap-2"><Input aria-label="Box 名称" maxLength={200} onChange={(event) => setName(event.target.value)} value={name} /><Button disabled={pending || !name.trim() || name.trim() === box.name} onClick={() => onRename(name.trim())} size="sm"><Pencil className="size-3.5" />保存</Button></div>
        <div className="flex flex-wrap gap-2 border-t pt-4">
          {box.status !== "revoked" ? <Button disabled={pending} onClick={() => onRevoke("drain")} size="sm" variant="outline"><Unplug className="size-3.5" />等待任务完成后撤销</Button> : null}
          {box.status !== "revoked" ? <Button className="bg-destructive text-destructive-foreground hover:bg-destructive/90" disabled={pending} onClick={forceRevoke} size="sm"><ShieldAlert className="size-3.5" />强制撤销</Button> : null}
        </div>
      </CardContent>
    </Card>
  );
...
function Metric({ label, value }: Readonly<{ label: string; value: string }>) {
  return <div><p className="text-muted-foreground">{label}</p><p className="mt-0.5 break-all font-medium">{value}</p></div>;
}
