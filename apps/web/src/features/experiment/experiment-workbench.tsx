...
import { ComparisonPanel, ExperimentCard, LabeledSelect } from "./experiment-ui";
...
export function ExperimentWorkbench() {
  ...
  return (
    <section aria-labelledby="experiments-title" className="space-y-6">
      <header>
        <div className="mb-3 flex size-10 items-center justify-center rounded-lg border border-border bg-card shadow-xs"><FlaskConical className="size-5" /></div>
        <h1 className="text-2xl font-semibold" id="experiments-title">实验</h1>
        <p className="mt-1 text-sm text-muted-foreground">实验列表以卡片展示状态和进度；点击卡片进入独立详情页查看 Terminal 与结果文件树。</p>
      </header>

      <Card>
        <CardHeader><CardTitle>新建实验</CardTitle><CardDescription>Runtime 策略和 Box 只在托管运行时生效，确认运行后配置不可修改。</CardDescription></CardHeader>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button onClick={() => { setCompareMode((value) => !value); setCompareIds([]); }} size="sm" variant={compareMode ? "default" : "outline"}><GitCompareArrows className="size-4" />{compareMode ? "退出比较" : "比较实验"}</Button>
      </div>
      {compareMode ? <ComparisonPanel items={comparison.data?.items ?? []} selectedCount={compareIds.length} /> : null}

      <div className="space-y-3">
        {experiments.data?.items.map((item) => <ExperimentCard checked={compareIds.includes(item.experiment_id)} compareMode={compareMode} item={item} key={item.experiment_id} onCancel={() => action.mutate({ id: item.experiment_id, kind: "cancel" })} onCompare={() => toggleCompare(item.experiment_id)} onRerun={() => action.mutate({ id: item.experiment_id, kind: "rerun" })} onRun={() => action.mutate({ id: item.experiment_id, kind: "run" })} />)}
        {!experiments.isLoading && !experiments.data?.items.length ? <p className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">尚无实验记录。</p> : null}
      </div>
    </section>
  );
}
