...
export function ProgressItemCard({
  ...
  const completed =
    kind === "task"
      ? (item as ProgressTask).status === "done"
      : (item as ProgressMilestone).status === "completed";
  const task = kind === "task" ? (item as ProgressTask) : null;
  const start = task?.start_at
    ? new Date(task.start_at)
    : (item as ProgressMilestone).target_at
      ? new Date((item as ProgressMilestone).target_at!)
      : null;
    ...
    >
      <div className="flex min-w-0 items-start gap-2">
        ...
        {task ? (
          <WorkState
            status={
              task.work_state ?? (task.status === "done" ? "todo" : task.status)
            }
            compact={compact}
          />
        ) : null}
        {canManage ? (
          <button
            aria-label={`删除${kind === "task" ? "任务" : "关键节点"} ${item.title}`}
...
function CompletionControl({
  ...
  if (proposal) {
    return (
      ...
      >
        <button
          aria-label={`认可 ${label} 已完成`}
          className="rounded-full bg-amber-500 p-0.5 text-white hover:bg-amber-600 disabled:opacity-50"
          disabled={!canManage}
          onClick={() => onReview(proposal, "accepted")}
          type="button"
        >
          <Check aria-hidden="true" className="size-3.5" />
        </button>
        <button
          aria-label={`不认可 ${label} 已完成`}
          className="rounded-full border border-amber-500 bg-background/60 p-0.5 text-amber-700 hover:bg-background disabled:opacity-50"
          disabled={!canManage}
          onClick={() => onReview(proposal, "rejected")}
          type="button"
        >
  ...
  return (
    <button
      aria-label={completed ? `将 ${label} 标为未完成` : `将 ${label} 标为完成`}
    ...
    >
      {completed ? (
        <CircleCheckBig aria-hidden="true" className="size-4 fill-current" />
      ) : (
...
function WorkState({
  status,
}: Readonly<{ compact: boolean; status: ProgressTask["status"] }>) {
  const values = {
    todo: { icon: CircleDotDashed, label: "待办", style: "text-slate-500" },
    in_progress: {
      icon: LoaderCircle,
      label: "进行中",
      style: "text-blue-600 dark:text-blue-300",
    },
    blocked: {
      icon: OctagonAlert,
      label: "阻塞",
      style: "text-red-600 dark:text-red-300",
    },
