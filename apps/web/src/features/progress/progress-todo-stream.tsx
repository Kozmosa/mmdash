...
export function ProgressTodoStream(props: Readonly<Props>) {
  ...
  return (
    <section aria-label="TODO 时间流" className="flex h-full max-h-[calc(100dvh-7rem)] min-h-0 min-w-0 select-none flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
...
function TodoDay(props: Readonly<Props & { day: Date; periodMode: boolean }>) {
  ...
  return <section className="overflow-hidden rounded-lg border border-border">
    <div className={cn("sticky top-0 z-10 flex items-center justify-between border-b border-border bg-card/95 px-4 py-3 backdrop-blur", today && "text-red-500")}><h3 className="text-base font-semibold">{formatShortDay(day)}</h3><span className="text-xs opacity-60">{taskValues.filter((task) => task.status !== "done").length} 项待完成</span></div>
    <div className="border-b border-border bg-muted/20 px-4 py-2" onDoubleClick={(event) => { if (canCreateFromDoubleClick(event)) onCreate(dateAtMinutes(day, 9 * 60), "milestone", false); }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => {
...
function TodoSection({ allTasks, canManage, completionByTarget, day, label, onCreate, onDeleteTask, onMoveTask, onOpenTask, onReview, onToggleTask, periodStart, showCurrentTime, tasks }: Readonly<Omit<Props, "tasks"> & { allTasks: ProgressTask[]; day: Date; label: string; periodStart?: number; showCurrentTime: boolean; tasks: ProgressTask[] }>) {
  ...
  return <div className="min-h-20 px-4 py-3" onDoubleClick={(event) => { if (canCreateFromDoubleClick(event)) onCreate(dateAtMinutes(day, periodStart ?? 9 * 60), "task", true); }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => moveHere(event.dataTransfer.getData("application/x-mmdash-task"))}>
...
function canCreateFromDoubleClick(event: React.MouseEvent<HTMLElement>): boolean {
  const target = event.target;
  return target instanceof HTMLElement && !target.closest("[data-progress-kind],button,a,input,textarea,select");
}
