...
export function ProgressItemDrawer({
  busy,
  onClose,
  onSave,
  selection,
  ...
  useEffect(() => {
    ...
    if ("task" in selection) {
      ...
      setEnd(
        selection.kind === "task"
          ? toLocalInput(new Date(at.getTime() + 60 * 60_000).toISOString())
          : "",
      );
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (kind === "task" && (!startAt || !endAt || endAt <= startAt))
      return setError("任务需要有效的开始和结束时间。");
    if (kind === "milestone" && !startAt)
      return setError("请选择关键节点所在日期。");
    onSave({
      ...
      <aside className="h-full w-full max-w-md overflow-y-auto border-l border-border bg-background p-6 shadow-2xl">
        ...
        <form className="mt-6 space-y-5" onSubmit={submit}>
            ...
            <fieldset>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button
                  ...
                  variant={kind === "task" ? "secondary" : "outline"}
                >
                  任务安排
                </Button>
                <Button
                  aria-pressed={kind === "milestone"}
                  onClick={() => setKind("milestone")}
                  variant={kind === "milestone" ? "secondary" : "outline"}
                >
                  关键节点
                </Button>
              </div>
          ...
          <label className="block space-y-2 text-sm">
            <span>说明</span>
            <textarea
              className="min-h-36 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
              maxLength={10_000}
              onChange={(event) => setDescription(event.target.value)}
              placeholder="记录交付标准、背景或下一步…"
              value={description}
            />
          </label>
          <label className="block space-y-2 text-sm">
            <span>{kind === "task" ? "开始" : "日期与时刻"}</span>
            <Input
          ...
          {kind === "task" ? (
            ...
            <label className="flex items-center justify-between gap-3 rounded-lg border border-border p-3 text-sm">
              <span>
                <span className="block font-medium">包含具体时刻</span>
                <span className="text-xs text-muted-foreground">
                  关闭后只显示在节点区
                </span>
              </span>
              <input
                checked={targetHasTime}
                onChange={(event) => setTargetHasTime(event.target.checked)}
                type="checkbox"
              />
            </label>
          )}
