...
export function parseSyncTex(
  ...
  for (const rawLine of source.split(/\r?\n/u)) {
    ...
    const record =
      /^[[(vhxkg$]?\s*(\d+),(\d+):(?:-?\d+,)?(-?\d+),(-?\d+)/u.exec(rawLine);
    if (!record) continue;
  ...
  for (const point of points)
    unique.set(`${point.file}:${point.line}:${point.page}:${point.y}`, point);
...
export function forwardSyncPoint(
  ...
  return points
    ...
    .reduce<SyncTexPoint | undefined>(
      (best, point) =>
        !best || Math.abs(point.line - line) < Math.abs(best.line - line)
          ? point
          : best,
      undefined,
    );
...
export function reverseSyncPoint(
  ...
  const target =
    min + (max - min) * Math.min(1, Math.max(0, verticalPercent / 100));
