...
describe("SyncTeX navigation", () => {
  it("maps generated records to immutable source files in both directions", () => {
    ...
    expect(forwardSyncPoint(points, "main.tex", 18)).toMatchObject({ line: 20, page: 1 });
    expect(reverseSyncPoint(points, 1, 100)).toMatchObject({ file: "sections/results.tex", line: 8 });
    expect(reverseSyncPoint(points, 2, 50)).toMatchObject({ file: "sections/results.tex", line: 30 });
  });
});
