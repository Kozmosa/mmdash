...
export function inspectOverleafBytes(
  ...
  const candidates = Object.entries(entries)
    .filter(
      ([name, value]) =>
        name.toLowerCase().endsWith(".tex") &&
        decode(value).includes("\\begin{document}"),
    )
    .map(([name]) => name)
    .sort((left, right) =>
      left === "main.tex"
...
export async function convertOverleafZip(
  ...
  fields: Pick<
    ...
    "name" | "version" | "engine" | "bibliography_tool"
  >,
): Promise<{ file: File; manifest: ArticleTemplateManifest }> {
  const converted = convertOverleafBytes(
    file.name,
  ...
  return {
    file: new File(
      [
        converted.bytes.buffer.slice(
          converted.bytes.byteOffset,
          converted.bytes.byteOffset + converted.bytes.byteLength,
        ) as ArrayBuffer,
      ],
      `${basename(file.name, ".zip")}-mmdash.zip`,
...
export function convertOverleafBytes(
  ...
  if (entries["mmdash-template.json"])
    throw new Error("该 ZIP 已包含 mmdash manifest，请按标准模板直接注册");
  const tex = decode(source);
  const begin = tex.indexOf("\\begin{document}");
  const end = tex.lastIndexOf("\\end{document}");
  if (begin < 0 || end <= begin)
    throw new Error("TeX 主文件缺少完整 document 环境");
  const contentTarget = ".mmdash/generated-content.tex";
  const bibliographyTarget = ".mmdash/references.bib";
  if (entries[contentTarget] || entries[bibliographyTarget])
    throw new Error("ZIP 使用了 mmdash 保留的生成路径");
  ...
  const manifest: ArticleTemplateManifest = {
    ...
    bibliography_target: bibliographyTarget,
    engine: fields.engine,
    bibliography_tool: fields.bibliography_tool,
  };
  if (!manifest.name || !manifest.version)
    throw new Error("模板名称和版本不能为空");
  ...
  entries["mmdash-template.json"] = strToU8(
    `${JSON.stringify(manifest, undefined, 2)}\n`,
  );
  const converted = zipSync(
    Object.fromEntries(
...
function readArchive(
  ...
  if (
    ...
    throw new Error("请选择不超过 128 MiB 的 ZIP");
  ...
  if (
    ...
    totalBytes(entries) > MAX_EXPANDED_BYTES
  )
    throw new Error("ZIP 文件数量或解压大小超过限制");
  const folded = new Set<string>();
  for (const name of names) {
    if (!safePath(name) || FORBIDDEN.test(name))
      throw new Error(`ZIP 包含不安全文件：${name}`);
...
function decode(value: Uint8Array) {
  try {
    ...
    throw new Error("TeX 主文件不是 UTF-8 文本");
  }
}
