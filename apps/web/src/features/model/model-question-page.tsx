...
const ArtifactDetailDrawer = dynamic(
  () =>
    import("@/features/artifact/artifact-detail-drawer").then(
      (module) => module.ArtifactDetailDrawer,
    ),
  { ssr: false },
);
...
export function ModelQuestionPage({ questionId }: { questionId: string }) {
  ...
  const detail = useQuery({
    ...
    refetchInterval: (query) =>
      ["queued", "running"].includes(
        (query.state.data as ModelQuestionDetail | undefined)?.question
          .sync_status ?? "",
      )
        ? 1_500
        : false,
  ...
  const selectedSummary = detail.data?.snapshots.find(
    (item) => item.snapshot_id === activeId,
  );
  const snapshot = useQuery({
    enabled: Boolean(
      activeId && activeId !== detail.data?.latest_snapshot?.snapshot_id,
    ),
    queryFn: () =>
      apiClient.request<ModelSnapshot>(
        `${base}/snapshots/${encodeURIComponent(activeId!)}`,
      ),
    queryKey: ["model-snapshot", project.id, questionId, activeId],
  });
  const activeSnapshot =
    detail.data && activeId === detail.data.latest_snapshot?.snapshot_id
      ? detail.data.latest_snapshot
      : snapshot.data;
  const diff = useQuery({
    enabled: Boolean(
      showDiff && diffFromId && diffToId && diffFromId !== diffToId,
    ),
    queryFn: () =>
      apiClient.request<ModelDiff>(`${base}/diff`, {
        query: { from_snapshot_id: diffFromId, to_snapshot_id: diffToId },
      }),
  ...
  const sync = useMutation({
    mutationFn: () => apiClient.request(`${base}/sync`, { method: "POST" }),
    onSuccess: async () => {
      setSelectedId(undefined);
      await invalidate();
      toast.success("模型同步已进入队列，自动同步倒计时已重置");
    },
  });

  if (detail.isLoading)
    return <p className="text-sm text-muted-foreground">正在读取模型版本…</p>;
  if (detail.error || !detail.data)
    return (
      <p className="text-sm text-destructive">
        {detail.error?.message ?? "模型不存在"}
      </p>
    );
  ...
  const toggleDiff = () => {
    ...
    const to =
      activeId ??
      detail.data.question.latest_snapshot_id ??
      detail.data.snapshots[0]?.snapshot_id;
    const from =
      selectedSummary?.previous_snapshot_id ??
      detail.data.snapshots.find((item) => item.snapshot_id !== to)
        ?.snapshot_id;
    setDiffFromId(from);
    setDiffToId(to);
    setShowDiff(true);
    ...
    <>
      <section className="space-y-4" aria-labelledby="model-question-title">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <Link
              ...
              href={`/projects/${encodeURIComponent(project.id)}/models`}
            >
            ...
            <div className="flex flex-wrap items-center gap-2">
              <h1
                ...
                id="model-question-title"
              >
                {question.code} · {question.title}
              </h1>
              <Badge>{question.sync_status}</Badge>
            </div>
            <a
              className="mt-1 inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground"
              href={question.notion_page_url}
              rel="noreferrer"
              target="_blank"
            >
              打开 Notion 页面
        ...
        {!detail.data.snapshots.length ? (
          <EmptyState
            description="点击右上角同步；Worker 读取绑定的 Notion 子页面后会生成第一个不可变快照。"
            title="尚无模型版本"
          />
        ) : (
          <div className="grid items-start gap-4 xl:grid-cols-[minmax(10rem,1fr)_minmax(0,6fr)_minmax(16rem,3fr)]">
            <aside className="space-y-2 xl:sticky xl:top-4">
              ...
              {detail.data.snapshots.map((item, index) => (
                <button
                  className={`w-full rounded-lg border p-3 text-left transition-colors ${item.snapshot_id === activeId ? "border-primary bg-primary/5" : "border-border hover:bg-muted/50"}`}
                  key={item.snapshot_id}
                  onClick={() => {
                    setSelectedId(item.snapshot_id);
                    setShowDiff(false);
                  }}
                  type="button"
                >
                  <span className="block text-sm font-medium">
                    版本 {detail.data.snapshots.length - index}
                  </span>
              ...
              <Card className="overflow-hidden">
                <CardHeader className="space-y-3 border-b border-border">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <CardTitle>
                        {activeSnapshot?.title ?? selectedSummary?.title}
                      </CardTitle>
                    ...
                    <Button
                      disabled={detail.data.snapshots.length < 2}
                      onClick={toggleDiff}
                      size="sm"
                  ...
                  {showDiff ? (
                    <DiffVersionPicker
                      ...
                      snapshots={detail.data.snapshots}
                      toId={diffToId}
                    />
                  ) : null}
                </CardHeader>
                <CardContent className="p-0">
                  <article className="mx-auto max-w-3xl px-8 py-10 sm:px-12">
                    {showDiff ? (
                      <DiffDocument
                        diff={diff.data}
                        invalid={
...
function SnapshotInfo({
  ...
  useEffect(() => {
    ...
    setNote(snapshot?.version_note ?? "");
  }, [snapshot?.snapshot_id, snapshot?.tags, snapshot?.version_note]);
  const save = useMutation({
    mutationFn: () =>
      apiClient.request(
        `${base}/snapshots/${encodeURIComponent(snapshot!.snapshot_id)}`,
        {
          body: {
            tags: tags
              .split(/[,，]/)
  ...
  return (
    <aside className="space-y-4 xl:sticky xl:top-4">
      <Card>
        <CardHeader>
          <CardTitle className="text-base">文档信息</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <InfoRow
            icon={FileArchive}
            label="内容 Hash"
            value={snapshot.content_hash.slice(0, 12)}
          />
          <InfoRow
            icon={FileArchive}
            label="模型文件"
            value={`${snapshot.assets.length} 个`}
          />
          <div className="space-y-2">
            <label className="text-xs font-medium" htmlFor="model-tags">
              Tags
            </label>
            <Input
              id="model-tags"
              onChange={(event) => setTags(event.target.value)}
              placeholder="初稿, 自定义标签"
              value={tags}
            />
            <div className="flex flex-wrap gap-1">
              {builtInTags.map((value) => (
                <Button
                  key={value}
                  onClick={() =>
                    setTags((current) =>
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium" htmlFor="version-note">
              版本说明
            </label>
            <textarea
      ...
      <DocumentOutline outline={snapshot.outline} />
    </aside>
  );
}
...
function DocumentOutline({ outline }: { outline?: ModelSnapshot["outline"] }) {
  const items = (outline ?? []).filter(
    (item) => item.level >= 1 && item.level <= 3 && item.title.trim(),
  );
  return (
            ...
            <ol className="space-y-1">
              {items.map((item) => (
                <li key={item.block_id}>
                  <a
                    className="block truncate rounded px-2 py-1.5 text-sm text-muted-foreground hover:bg-muted hover:text-foreground"
                    href={`#${headingAnchor(item.block_id)}`}
                    style={{
...
function DiffVersionPicker({
  ...
  snapshots: ModelQuestionDetail["snapshots"];
  toId?: string;
}) {
  const optionLabel = (index: number, capturedAt: string) =>
    `版本 ${snapshots.length - index} · ${new Date(capturedAt).toLocaleString()}`;
  return (
    <div className="grid gap-3 rounded-lg bg-muted/50 p-3 sm:grid-cols-[1fr_auto_1fr]">
      <label className="space-y-1 text-xs font-medium">
        <span>基准版本</span>
        <select
          aria-label="Diff 基准版本"
        ...
        >
          ...
          {snapshots.map((item, index) => (
            <option
              disabled={item.snapshot_id === toId}
              key={item.snapshot_id}
              value={item.snapshot_id}
            >
              {optionLabel(index, item.captured_at)}
            </option>
          ))}
        </select>
      </label>
      <span className="self-end pb-2 text-center text-xs text-muted-foreground">
        比较到
      </span>
      <label className="space-y-1 text-xs font-medium">
        <span>目标版本</span>
        <select
          aria-label="Diff 目标版本"
          className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm"
        ...
        >
          ...
          {snapshots.map((item, index) => (
            <option
              disabled={item.snapshot_id === fromId}
              key={item.snapshot_id}
              value={item.snapshot_id}
            >
              {optionLabel(index, item.captured_at)}
...
export function ModelDocument({
  ...
  const assetsByBlock = useMemo(
    () => new Map(assets.map((asset) => [asset.source_block_id, asset])),
    [assets],
  );
  return (
    <ModelBlocks
      assetsByBlock={assetsByBlock}
      blocks={blocks}
...
function ModelBlocks({
  ...
  assetsByBlock: Map<string, ModelAsset>;
  blocks: ModelBlock[];
  onArtifactOpen: (artifactId: string) => void;
  projectId: string;
}) {
...
function extractRichTextHtml(parts: ModelRichText[] = []): string {
  return parts
    .map((part) => {
      ...
      if (part.href)
        content = `<a href="${escapeHtml(part.href)}">${content}</a>`;
      return content;
    })
    .join("");
}
...
function ModelBlockView({
  ...
  const content = (
    <>
      {block.rich_text.length
        ? block.rich_text.map((part, index) => (
            <RichTextView key={`${block.block_id}-${index}`} part={part} />
          ))
        : block.text}
    </>
  );
  ...
  const artifactVersionId =
    block.artifact_version_id ?? asset?.artifact_version_id;
  const detail = useQuery({
  ...
  const renamed = Boolean(
    detail.data?.artifact?.name &&
    asset?.filename &&
    detail.data.artifact.name !== asset.filename,
  );
  ...
  const documentTitle =
    ...
    (block.caption ||
      asset?.filename ||
      (block.type === "pdf" ? "Notion PDF" : "Notion 文件"));
  let node;
  if (block.type.startsWith("heading_")) {
    const level = block.level ?? 1;
    const id = headingAnchor(block.block_id);
    node =
      level === 1 ? (
        <h1 className="mb-2 mt-7 scroll-mt-6 text-3xl font-bold" id={id}>
          {content}
        </h1>
      ) : level === 2 ? (
        <h2 className="mb-1 mt-6 scroll-mt-6 text-2xl font-semibold" id={id}>
          {content}
        </h2>
      ) : (
        <h3 className="mb-1 mt-5 scroll-mt-6 text-xl font-semibold" id={id}>
          {content}
        </h3>
      );
    ...
    node = (
      <label className="flex gap-3 py-1">
    ...
    node = (
      <blockquote className="my-2 border-l-4 border-foreground pl-4">
        {content}
      </blockquote>
    );
    ...
    node = (
      <MathExpression displayMode expression={block.expression ?? block.text} />
    );
  else if (block.type === "table") {
    const rows = block.children.map(
      (child) =>
        child.cells ?? (child.rows?.[0] ?? []).map((text) => [{ text }]),
    );
    node = (
              ...
              <tr key={`${block.block_id}-row-${rowIndex}`}>
                {row.map((cell, cellIndex) => (
                  <td
                    ...
                    key={`${block.block_id}-${rowIndex}-${cellIndex}`}
                  >
                    {cell.map((part, partIndex) => (
                      <RichTextView
                        key={`${block.block_id}-${rowIndex}-${cellIndex}-${partIndex}`}
                        part={part}
                      />
                    ))}
                  </td>
    ...
    node = (
      <a
        className="my-3 flex items-center justify-between gap-4 rounded-lg border border-border p-4 hover:bg-muted/50"
        href={block.url}
        rel="noreferrer"
        target="_blank"
      >
        <span className="min-w-0">
          <span className="block truncate font-medium">
            {block.caption || bookmarkTitle(block.url)}
          </span>
    ...
    node = block.children.length ? null : (
      <p className="my-3 rounded-lg border border-dashed border-border p-4 text-sm text-muted-foreground">
        同步区块尚未读取到内容，请重新同步。
      </p>
    );
    ...
    node = (
      <ModelArtifactImage
        ...
        projectId={projectId}
        title={
          displayName ?? (block.caption || asset?.filename || "Notion 模型图片")
        }
      />
    );
  else if (["file", "pdf"].includes(block.type))
    node = (
      <button
        className="my-3 flex w-full items-center gap-3 rounded-lg border border-border p-4 text-left hover:bg-muted/50"
        disabled={!artifactId}
        onClick={() => artifactId && onArtifactOpen(artifactId)}
        type="button"
      >
        <FileArchive className="size-5 shrink-0" />
        <span className="min-w-0">
          <span className="block truncate font-medium">{documentTitle}</span>
          {block.caption &&
          asset?.filename &&
          block.caption !== asset.filename ? (
    ...
    node = <hr className="my-5 border-border" />;
  else
    node = (
      <p className="min-h-6 whitespace-pre-wrap py-1 leading-7">{content}</p>
    );

  const canCopy =
    block.type !== "divider" &&
    !(block.type === "synced_block" && !block.children.length);

  return (
    <div className="group relative" data-block-id={block.block_id}>
      <div className="min-w-0 flex-1 pr-6">{node}</div>
      {canCopy ? (
        <ModelBlockCopyButton
          asset={asset}
          block={block}
        ...
        <div
          className={
            block.type === "synced_block"
              ? ""
              : "ml-6 border-l border-border pl-4"
          }
        >
          <ModelBlocks
            assetsByBlock={assetsByBlock}
...
function headingAnchor(blockId: string) {
  return `model-heading-${blockId}`;
}
...
function ModelArtifactImage({
  ...
  const download = useQuery({
    enabled: Boolean(artifactId),
    queryFn: () =>
      artifactApi.download(projectId, artifactId!, artifactVersionId),
    queryKey: [
  ...
  return (
    <figure className="my-4">
      <button
        ...
        className="block w-full overflow-hidden rounded-lg border border-border bg-muted/20 hover:border-foreground/30"
      ...
      >
        {download.data?.transfer.url ? (
          ...
          <span className="flex min-h-40 items-center justify-center p-6 text-sm text-muted-foreground">
            {download.error
              ? "图片加载失败，点击查看 Artifact 信息"
              : "正在加载模型图片…"}
          </span>
        )}
      </button>
  ...
  if (part.expression)
    return (
      <span className={className}>
        <MathExpression expression={part.expression} />
      </span>
    );
  return part.href ? (
    <a
...
function MathExpression({
  ...
  try {
    ...
    return displayMode ? (
      <div className="my-3 overflow-x-auto text-center font-mono text-sm">
        {expression}
      </div>
    ) : (
  ...
  return displayMode ? (
    <div
      ...
      data-model-equation="block"
      dangerouslySetInnerHTML={markup}
    />
  ) : (
    <span
      className="inline-block max-w-full align-middle"
      data-model-equation="inline"
      dangerouslySetInnerHTML={markup}
    />
  );
}
...
function DiffDocument({
  ...
  if (loading)
    return <p className="text-sm text-muted-foreground">正在生成字符级差异…</p>;
  if (!diff)
    return <p className="text-sm text-muted-foreground">正在读取版本差异…</p>;
  return (
    <div className="space-y-1">
      {diff.blocks.map((block) => (
        <DiffBlockView diffBlock={block} key={block.block_id} />
      ))}
    </div>
  );
}
...
function DiffBlockView({ diffBlock }: { diffBlock: ModelDiffBlock }) {
  ...
  const content = (
    <DiffRuns blockId={block.block_id} operations={diffBlock.operations} />
  );
  const changeClass =
    diffBlock.change === "added"
      ? "border-l-2 border-blue-400 bg-blue-50/40 pl-3 dark:bg-blue-950/20"
      : diffBlock.change === "deleted"
        ? "border-l-2 border-pink-300 bg-pink-50/40 pl-3 dark:bg-pink-950/20"
        : "";
  let node;
  if (block.type.startsWith("heading_")) {
    const level = block.level ?? 1;
    node =
      level === 1 ? (
        <h1 className="mb-2 mt-7 text-3xl font-bold">{content}</h1>
      ) : level === 2 ? (
        <h2 className="mb-1 mt-6 text-2xl font-semibold">{content}</h2>
      ) : (
        <h3 className="mb-1 mt-5 text-xl font-semibold">{content}</h3>
      );
  } else if (block.type === "bulleted_list_item")
    node = (
      <div className="flex gap-3 py-1">
    ...
    node = (
      <div>
        <MathExpression
          displayMode
          expression={block.expression ?? block.text}
        />
        {diffBlock.change !== "unchanged" ? (
    ...
    const cells = splitDiffCells(diffBlock.operations);
    node = (
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-sm">
          <tbody>
            <tr>
              {cells.map((operations, index) => (
                <td
                  className="border border-border px-3 py-2 align-top"
                  key={`${block.block_id}-cell-${index}`}
                >
                  <DiffRuns
                    blockId={`${block.block_id}-${index}`}
  ...
  else if (["image", "file", "pdf"].includes(block.type))
    node = (
      <div className="my-3 flex items-center gap-3 rounded-lg border border-border p-4">
        <FileArchive className="size-5" />
        <span>
          {block.caption || (block.type === "image" ? "模型图片" : "模型文件")}
        </span>
      </div>
    );
  else if (block.type === "divider")
  ...
  return (
    <div className={changeClass} data-diff-change={diffBlock.change}>
      {node}
    </div>
  );
}
        ...
        <span
          className={
            operation.kind === "added"
              ? "rounded-sm bg-blue-100 px-0.5 text-blue-950 dark:bg-blue-900/60 dark:text-blue-100"
              : operation.kind === "deleted"
                ? "rounded-sm bg-pink-100 px-0.5 text-pink-700/60 line-through dark:bg-pink-900/40 dark:text-pink-200/60"
                : ""
          }
          data-diff-kind={operation.kind}
          key={`${blockId}-${index}`}
        >
          {operation.text}
        </span>
