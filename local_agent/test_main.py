...
@pytest.mark.anyio
async def test_experiment_run_success_with_param_grid_creates_complete_artifacts(tmp_path: Path):
    repo_path = tmp_path / "repo"
    repo_path.mkdir()
    init_git_repo(repo_path)
    solver_path = repo_path / "solver_ok.py"
    solver_path.write_text(
        "\n".join(
            [
                "import os",
    ...
    assert all(item["returncode"] == 0 for item in response["data"]["results"])
    assert response["data"]["results"][0]["params"] == {"alpha": "3"}
    assert response["data"]["results"][1]["params"] == {"alpha": "4"}
    result_dir = Path(response["data"]["result_dir"])
    assert (result_dir / "fig").is_dir()
    assert (result_dir / "log.txt").exists()
    assert (result_dir / "analysis.md").exists()
    assert (result_dir / "params_snapshot.json").exists()
...
@pytest.mark.anyio
async def test_experiment_run_failure_preserves_result_bundle(tmp_path: Path):
    repo_path = tmp_path / "repo"
    repo_path.mkdir()
    init_git_repo(repo_path)
    solver_path = repo_path / "solver_fail.py"
    solver_path.write_text(
        "\n".join(
            [
                "import sys",
    ...
    assert response["data"]["task"]["status"] == "failed"
    assert response["data"]["results"][0]["returncode"] == 3
    result_dir = Path(response["data"]["result_dir"])
    assert (result_dir / "log.txt").exists()
    assert (result_dir / "analysis.md").exists()
    assert (result_dir / "params_snapshot.json").exists()
