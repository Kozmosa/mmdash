...
-- Stage 1 development branches previously shipped this identical schema as
-- 000010_repo. The IF NOT EXISTS guards let those preserved databases record
-- the post-merge 000012 name without rebuilding or losing Repo data.
...
CREATE INDEX IF NOT EXISTS repo_repositories_sync_claim_idx
...
CREATE INDEX IF NOT EXISTS repo_repositories_cleanup_idx
...
CREATE INDEX IF NOT EXISTS repo_commits_timeline_idx
    ON repo_commits (repository_id, committer_time DESC, commit_sha DESC);

CREATE TABLE IF NOT EXISTS repo_commit_events (
    repository_id UUID NOT NULL,
    workspace_kind TEXT NOT NULL,
    commit_sha TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS repo_checkouts_expiry_idx
    ON repo_checkouts (status, expires_at)
    WHERE status IN ('active', 'error');

CREATE TABLE IF NOT EXISTS repo_commit_requests (
    repository_id UUID NOT NULL,
    workspace_kind TEXT NOT NULL,
