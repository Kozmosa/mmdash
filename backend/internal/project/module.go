...
func (module Module) handleResource(response http.ResponseWriter, request *http.Request) {
	...
	switch segments[1] {
	case "article":
		if module.Article != nil {
			module.Article.ServeHTTP(response, request)
			return
		}
	...
	case "experiments":
		if module.Experiment != nil {
			module.Experiment.ServeHTTP(response, request)
			return
		}
	case "artifacts":
